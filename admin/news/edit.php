...
<body>
    ...
    if (isset($_POST['submit'])) {
        ...
        $edit = mysqli_query($conn, "UPDATE news SET title='$title', conclusion='$conclusion', category_news='$category_news' WHERE id='$id'");
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="/project/admin/news">News</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Overview</li>
                    </ol>
                </nav>
                <h1 class="h2">Mengedit News</h1>
                <p>Ini adalah halaman update News.</p>
                <div class="card">
                    <div class="card-body">
                        <?php

$id_news = $_GET['id'];
$sql = mysqli_query($conn, "SELECT * FROM news WHERE id_news='$id_news'");

while($data = mysqli_fetch_array($sql)){
    $id = $data ['id_news'];
    $title = $data ['title'];
    $conclusion = $data ['conclusion'];
    $category_news = $data ['category_news'];

?>

                        <form action="<?php echo $_SERVER['PHP_SELF'] . " ?id=" . $id_news ?>" method="post"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="conclusion" name="conclusion"
                                    placeholder="Conclusion" required value="<?=$conclusion;?>">
                            </div>
                            <div class="mb-3">
                                <label for="category_news" class="form-label">Category</label>
                                <input type="text" class="form-control" id="category_news" name="category_news"
                                    placeholder="category_news" required value="<?=$category_news;?>">
                            </div>
