...
<body>
    ...
    $id = $_GET['id'];
    $sql = "SELECT * FROM detail_news WHERE id='$id'";
    $datas = $conn->query($sql);

    while ($data = mysqli_fetch_array($datas)) {
        $title_news = $data['title_news'];
        $author_news = $data['author_news'];
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
                ...
                <h1 class="h2">Melihat News</h1>
                <p>Ini adalah halaman show News.</p>
                <div class="card">
                    <div class="card-body">
                        <form action="<?php echo $_SERVER['PHP_SELF'] . " ?id=" . $id ?>" method="post">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="title_news" name="title_news"
                                    placeholder="Title" required value="<?php echo $title_news ?>" disabled>
                            </div>
                            <div class="mb-3">
                                <label for="author_news" class="form-label">Author</label>
                                <input type="text" class="form-control" id="author_news" name="author_news"
                                    placeholder="author_news" required value="<?php echo $author_news ?>" disabled>
                            </div>
                            <div class="mb-3">
                                <label for="article" class="form-label">Article</label>
                                <input type="text" class="form-control" id="article" name="article"
                                    placeholder="article" required value="<?php echo $article ?>" disabled>
                            </div>
                            <div class="mb-3">
                                <label for="date" class="form-label">Tanggal_Publish</label>
                                <input type="date" class="form-control" id="date" name="date" placeholder="date"
                                    required value="<?php echo $date ?>" disabled>
                            </div>
                            <div class="mb-3">
                                <label for="time" class="form-label">Waktu_Publish</label>
                                <input type="time" class="form-control" id="time" name="time" placeholder="time"
                                    required value="<?php echo $time ?>" disabled>
                            </div>
                        </form>
                    </div>
                </div>
